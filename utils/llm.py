...
class LLMClient:
    """OpenAI-compatible chat client with optional adapter path and tracing."""
    ...
    def chat(self, messages: Union[str, List[Dict[str, str]]], stream: bool = False) -> Union[str, List[str]]:
        """Send chat messages; choose adapter if FEATURE_NEW_PIPELINE=1; trace I/O."""
        try:
    ...
    def from_env(cls, role: Optional[str] = None, dotenv_path: Optional[str] = None, temperature: float = 0.0, max_tokens: Optional[int] = None) -> "LLMClient":
        """Construct client from env variables for provider/base/model selection."""
        if dotenv_path:
